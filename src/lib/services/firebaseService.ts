...
export const carWashService = {
  ...
  async add(record: Omit<CarWashRecord, 'id'>): Promise<CarWashRecord | null> {
    try {
      ...
      // Подготавливаем данные для Firestore, убеждаясь что все поля имеют правильные типы
      const recordData = {
        date: record.date,
        time: record.time,
        carInfo: record.carInfo,
        ...
        paymentMethod: {
          type: record.paymentMethod.type,
          ...(record.paymentMethod.organizationId && {
            organizationId: record.paymentMethod.organizationId,
            organizationName: record.paymentMethod.organizationName
          })
        },
        employeeIds: record.employeeIds,
        createdAt: serverTimestamp()
      ...
      console.log('Отправляем в Firestore:', JSON.stringify(recordData));
...
export const appointmentService = {
  ...
  async add(appointment: Omit<Appointment, 'id'>): Promise<Appointment | null> {
    try {
      console.log('Добавление записи на мойку:', JSON.stringify(appointment));

      // Валидация данных
      if (!appointment.date || !appointment.time || !appointment.carInfo || !appointment.service) {
        console.error('Ошибка валидации: не все обязательные поля заполнены');
        return null;
      }
      ...
      console.log('Отправляем в Firestore (appointments):', JSON.stringify(appointmentData));

      const docRef = await addDoc(appointmentsRef, appointmentData);

      console.log('Запись на мойку успешно добавлена с ID:', docRef.id);
